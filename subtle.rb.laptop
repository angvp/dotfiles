...
screen 1 do
  ...
  top     [ :views, :center, :title, :volume, :notify, :tray, :clock ]

  # Content of the bottom panel
  bottom  [ :mpd, :separator, :pacman, :separator, :storage ]
  # bottom [ ]
end
...
# Exec programs
grab "W-Return", "urxvt"
@dmenu = "dmenu_run -fn 'xft:monaco-10' -nb '#181818' -nf '#989898' -sb '#181818' -sf '#FFCC00' -p 'Run : '"
# grab "W-F1", "@dmenu"
grab "W-F1", "gmrun"
grab "W-F2", "@dmenu"
# Screensaver lock
...
# Simple tags
tag "terms",   "xterm|[u]?rxvt|gmrun"
tag "browser", "uzbl|opera|firefox|navigator|gmrun"
...
tag "im" do
  match "pidgin|skype|gmrun"
  float false
end

tag "testing" do
  match "gnome-terminal|fceux|chromium|gmrun"
  float true
end

tag "mail" do
  match "thunderbird|gmrun"
  float true
end

tag "irc" do
  match "weechat-curses|xchat|gmrun"
  float true
end
